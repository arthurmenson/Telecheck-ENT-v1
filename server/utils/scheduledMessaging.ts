...
export class ScheduledMessagingService {
  ...
  async schedulePatientMessages(
    ...
    try {
      AuditLogger.log(
        "SYSTEM",
        "scheduled_messaging",
        `Scheduling messages for patient ${patientSchedule.patientId}`,
        {
      ...
      AuditLogger.log(
        "SYSTEM",
        "scheduled_messaging_error",
        `Failed to schedule messages for patient ${patientSchedule.patientId}`,
        {
  ...
  private async scheduleMedicationReminder(
    ...
    const job = schedule.scheduleJob(jobName, cronExpression, async () => {
      if (this.isQuietHours(time, patientSchedule.quietHours)) {
        console.log(
          `Skipping medication reminder during quiet hours: ${jobName}`,
        );
        return;
      }
  ...
  private async scheduleGlucoseReminder(
    patientSchedule: PatientSchedule,
    time: string,
  ): Promise<void> {
  ...
  private async scheduleCareTeamUpdates(
    patientSchedule: PatientSchedule,
  ): Promise<void> {
    ...
    switch (careTeamUpdates.frequency) {
      case "daily":
        cronExpression = this.timeToCron(
          careTeamUpdates.time,
          patientSchedule.timezone,
        );
        break;
      case "weekly":
      ...
      default:
        throw new Error(
          `Unsupported care team update frequency: ${careTeamUpdates.frequency}`,
        );
    }
  ...
  private async sendScheduledMessage(
    ...
    try {
      console.log(
        `Sending scheduled message: ${scheduledMessage.type} to ${scheduledMessage.phone}`,
      );
      ...
      if (scheduledMessage.retryCount < scheduledMessage.maxRetries) {
        ...
        AuditLogger.log(
          ...
          `Scheduled message failed after ${scheduledMessage.maxRetries} retries`,
          {
            scheduledMessageId: scheduledMessage.id,
            patientId: scheduledMessage.patientId,
  ...
  async cancelPatientSchedules(patientId: string): Promise<void> {
    ...
    console.log(
      `Cancelled ${jobsToCancel.length} scheduled jobs for patient ${patientId}`,
    );

    AuditLogger.log(
      "SYSTEM",
      "scheduled_messaging",
      `Cancelled scheduled messages for patient ${patientId}`,
      {
        patientId,
  ...
  async sendWellnessCheck(
    ...
    try {
      ...
      if (result.success) {
        ...
        console.error(
          `Failed to send wellness check to ${phone}:`,
          result.error,
        );
        return false;
      }
      ...
      console.error(
        `Error sending wellness check to patient ${patientId}:`,
        error,
      );
      return false;
    }
  ...
  private async updateScheduledMessageStatus(
      ...
      if (schedules && schedules.length > 0) {
        for (const scheduleRow of schedules) {
          try {
            ...
            console.error(
              `Error loading schedule for patient ${scheduleRow.patient_id}:`,
              error,
            );
          }
        }
  ...
  async pausePatientSchedule(patientId: string): Promise<void> {
    ...
    try {
      ...
      AuditLogger.log(
        "SYSTEM",
        "scheduled_messaging",
        `Paused schedule for patient ${patientId}`,
        {
          patientId,
        },
      );
    ...
    try {
      ...
      if (schedule) {
        ...
        AuditLogger.log(
          "SYSTEM",
          "scheduled_messaging",
          `Resumed schedule for patient ${patientId}`,
          {
            patientId,
          },
        );
