...
class InMemoryDatabase {
  ...
  private seedData() {
    ...
    const sampleInsights: HealthInsight[] = [
      {
        ...
        description:
          "Your LDL cholesterol is elevated. Consider discussing lifestyle modifications or medication adjustments with your healthcare provider.",
        priority: "medium",
      ...
      },
      {
        ...
        description:
          "Your fasting glucose levels are well within normal range, indicating good metabolic health.",
        priority: "low",
    ...
    sampleInsights.forEach((insight) =>
      this.healthInsights.set(insight.id, insight),
    );
  }
  ...
  getLabReports(userId: string): LabReport[] {
    return Array.from(this.labReports.values()).filter(
  ...
  getVitalSigns(userId: string): VitalSigns[] {
    return Array.from(this.vitalSigns.values()).filter(
      (vital) => vital.userId === userId,
    );
  }
